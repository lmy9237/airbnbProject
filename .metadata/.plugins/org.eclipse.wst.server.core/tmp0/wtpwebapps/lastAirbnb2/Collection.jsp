...
<%
	// Collection
	int cIdx = (Integer)request.getAttribute("cIdx");
	CollectionDao collectionDao = new CollectionDao();
	ExpCollectionVo cVo = (ExpCollectionVo)request.getAttribute("expcollectionVO");
	ArrayList<ExperienceVo> listExpVo = (ArrayList<ExperienceVo>)request.getAttribute("listColExp");
...
<html>
	<head>
		<meta charset="UTF-8">
		<title><%= cVo.getCollectionName() %></title>
	...
	<body>
		...
		<div id="main_image" style="background-image: url('<%= cVo.getImg()%>');">
			...
			<div><b><%= cVo.getCollectionName() %></b></div>
		</div>
		<script>
		$(function(){
			$(".logo").click(function(){
				location.href = 'Controller?command=main2';
			});
			$(".airbnb_area").click(function(){
				location.href = 'doairbnb.jsp';
			});
		});
		</script>
		<div id="filter">
		...
		<div id="title"><b><%= cVo.getCollectionName() %></b></div>
		
		<div class="expz">
			<% int cnt = 0; %>
			<% for(ExperienceVo vo : listExpVo) { %>
				<a class="exp" href="LswController?command=expInfo&exp_idx=<%=vo.getExpIdx()%>">
					<div class="exp_img" style="background-image: url('<%= vo.getExpImg1() %>')"></div>
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" aria-hidden="true" role="presentation" focusable="false" style="fill: rgba(0, 0, 0, 0.5);height: 24px;width: 24px; overflow: visible;"><path d="M16 28c7-4.73 14-10 14-17a6.98 6.98 0 0 0-7-7c-1.8 0-3.58.68-4.95 2.05L16 8.1l-2.05-2.05a6.98 6.98 0 0 0-9.9 0A6.98 6.98 0 0 0 2 11c0 7 7 12.27 14 17z"></path></svg>
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" aria-hidden="true" role="presentation" focusable="false" style="color: #222; height: 12px; width: 12px; fill: currentcolor;"><path fill-rule="evenodd" d="m15.1 1.58-4.13 8.88-9.86 1.27a1 1 0 0 0-.54 1.74l7.3 6.57-1.97 9.85a1 1 0 0 0 1.48 1.06l8.62-5 8.63 5a1 1 0 0 0 1.48-1.06l-1.97-9.85 7.3-6.57a1 1 0 0 0-.55-1.73l-9.86-1.28-4.12-8.88a1 1 0 0 0-1.82 0z"></path></svg>
					<span>5.0</span><span> <span>(</span><span>12</span><span>) &middot; <%= vo.getExpLocation() %></span></span>
					<div class="name"><%= vo.getExpName() %></div>
					<div class="price"><b>최저가 ₩</b><b><%= new DecimalFormat("###,###").format(vo.getExpPrice()) %></b> <span>/ 인</span></div>
				</a>
				<% 
					cnt++;
					if(cnt%5==0) {
