...
public class Main3Dao {
   ...
   public RoomVo getRoom(int roomIdx) {
      ...
      System.out.println("나중에 지워요 Main3Dao.java) roomvo==null : " + (roomvo==null));
         ...
         try {
            ...
            while(rs.next()) {
                ...
                if(room_info_cate_detail!=null && room_info_cate_detail.contains("(((n)))")) {
                   ...
                   String infoText = "";
                   if(rs2.next()) {
                	   infoText = rs2.getString("room_info_ex"); 
   ...
   // num : 11
   // getInside() ----> getRoomInfo
   private String getInside(String s, int num) { 
      String[] s2 = s.split("\\)");
   ...
   public ArrayList<ReviewVo> getReview(int roomIdx) {
	...
	// 후기검색어
	public ArrayList<ReviewVo> getSearch(int roomIdx, String searchText, String searchOrderBy){ //searchOrderBy : "최신순", "높은평점순"
		...
		try {
			...
			if("최신순".equals(searchOrderBy)) {
				sql += " ORDER BY written_date DESC";
			} else if("높은평점순".equals(searchOrderBy)) {
   ...
   // 날짜 막기 --> 가능한 날짜인지 리턴(true:예약가능함). / date8ch = (ex.)"20240218"
