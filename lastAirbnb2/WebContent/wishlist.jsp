...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    LanguageDao langDao = new LanguageDao();
    	...
    	ArrayList<WishListPageVo> wishList = wishListDao.getWishListByIdx(userIdx);
    %>
<!DOCTYPE html>
<html>
	<head>
		<meta charset="UTF-8">
		<title>내 위시리스트 · 위시리스트 - 에어비앤비</title>
               ...
               <div>
                  ...
                  <select name="jm_code" class="jm_seclect_box">
	               ...
	               <span>전화나 문자로 전화번호를 확인하겠습니다. 일반 문자 메시지 요금 및 데이터 요금이 부과됩니다.</span>
	         	   <input type="hidden" name="url" value="wishlist.jsp"/>
...
<!--회원가입창 끝  -->
		...
		<div id="content">
			<div><!--ììë¦¬ì¤í¸ ì ì²´ë¥¼ ê°ì¸ê³  ìë div-->
				<div><!--ììë¦¬ì¤í¸ ì ì²´ë¥¼ ê°ì¸ê³  ìë inner_div-->
					<h1>위시리스트</h1>
					<ul id="wishlist">
					<%if(wishList != null) { %>
					<% for(WishListPageVo vo : wishList) { %>
						<a href="#" class="fl">
							<button class="delete_button"><img src="images/delete.png"/></button>
							<img src="<%=vo.getImg1() == null ? "images/wishlist_img.png" : vo.getImg1() %>"/>
							<div><%=vo.getBigListName() %></div>
							<div><%=vo.getCountRoom() %>개 저장됨</div>
						</a>
					<% } //for문%>
					<% } else { %>
						<h1>로그인부터 해주세요</h1>
